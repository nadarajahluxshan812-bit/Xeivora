...
function buildLimitMessage({ plan = "Starter", used = 0, limit = 5 }) {
  ...
  for (const config of OPENAI_IMAGE_MODELS) {
    try {
      const batches = await Promise.all(
        Array.from({ length: requestedCount }, () => requestOpenAiImages(cleanPrompt, config))
      );
      ...
      if (images.length) {
        ...
        return {
          ...
          provider: "openai",
          model: config.model,
          modelLabel: config.model === "dall-e-3" ? "DALL-E 3" : config.model === "gpt-image-1" ? "GPT Image 1" : "DALL-E 2",
          attempts,
          count: images.length
        };
      }
