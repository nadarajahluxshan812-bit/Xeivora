...
async function upsertOAuthUser({ email, name, avatarUrl = null, provider = "google" }) {
  const normalizedEmail = normalizeEmail(email);
  const normalizedName = normalizeName(name) || normalizedEmail.split("@")[0] || "Xeivora User";

  if (!normalizedEmail) {
    const error = new Error(`${provider[0].toUpperCase()}${provider.slice(1)} account email is missing.`);
    error.statusCode = 400;
    throw error;
  }
  ...
  if (!isDatabaseConfigured()) {
    ...
    return createUser({ email: normalizedEmail, name: normalizedName, provider, avatarUrl });
  }
  ...
  const result = await query(
    `INSERT INTO xeivora_users
     ...
     VALUES ($1, $2, $3, $4, $5, 'Starter', $6::jsonb, $7, $7)
     ON CONFLICT (email)
     DO UPDATE SET
       name = EXCLUDED.name,
...
async function upsertGoogleUser({ email, name, avatarUrl = null }) {
  return upsertOAuthUser({ email, name, avatarUrl, provider: "google" });
}

async function upsertGitHubUser({ email, name, avatarUrl = null }) {
  return upsertOAuthUser({ email, name, avatarUrl, provider: "github" });
}
...
async function updateUserProfile(userId, updates = {}) {
  ...
  if (!isDatabaseConfigured()) {
    ...
    if (nextEmail && nextEmail !== user.email) {
      const emailTaken = jsonStore.users.some((entry) => entry.id !== userId && entry.email === nextEmail);
      if (emailTaken) {
        const error = new Error("An account with this email already exists.");
        error.statusCode = 409;
        throw error;
  ...
  if (nextEmail && nextEmail !== current.email) {
    const existing = await findUserRowByEmail(nextEmail);
