...
function normalizePreviewPayload(value) {
  ...
  const renderMode = renderModeMap[payload.renderMode] || null;
  ...
  const normalizedEndpoints = Array.isArray(payload.endpoints)
    ? payload.endpoints
        .map((endpoint) => {
          ...
          const method = sanitizeText(endpoint.method || "").toUpperCase();
          const pathValue = sanitizeText(endpoint.path || "");
          if (!method || !pathValue) {
            return null;
          }
          ...
          return {
            method,
            path: pathValue,
            description: sanitizeText(endpoint.description || "") || null
          };
        })
        .filter(Boolean)
        .slice(0, 12)
          ...
          const columns = Array.isArray(table.columns)
            ? table.columns
                .map((column) => {
                  ...
                  const columnName = sanitizeText(column.name || "");
                  const columnType = sanitizeText(column.type || "");
                ...
                .map((row) => {
                  ...
                  Object.entries(row).forEach(([key, cell]) => {
                    ...
                    if (typeof cell === "number") {
                      nextRow[normalizedKey] = Number.isFinite(cell) ? cell : null;
                    } else if (typeof cell === "string") {
                      nextRow[normalizedKey] = sanitizeText(cell) || "";
                    } else if (cell == null) {
                      nextRow[normalizedKey] = null;
                    }
                  });

                  return Object.keys(nextRow).length ? nextRow : null;
                })
                .filter(Boolean)
                .slice(0, 6)
            : [];
  ...
  return {
    ...
    exitCode: typeof payload.exitCode === "number" && Number.isFinite(payload.exitCode) ? payload.exitCode : null,
    ...
    mediaType:
      payload.mediaType === "image" || payload.mediaType === "video" || payload.mediaType === "pdf" || payload.mediaType === "slides"
        ? payload.mediaType
        : null,
