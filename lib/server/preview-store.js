...
async function createPreviewVersion(payload = {}) {
  const basePreview = {
    ...
    status: sanitizeText(payload.status || "live") || "live",
  ...
  await query(
    `INSERT INTO xeivora_preview_versions
      (id, project_id, session_id, version_number, title, summary, status, route_path, changed_files, notes, approved_at, deployed_at, created_at, updated_at)
...
async function updatePreviewVersion(previewId, payload = {}) {
  if (!isDatabaseConfigured()) {
    ...
    Object.assign(preview, {
      ...
      status: payload.status !== undefined ? sanitizeText(payload.status) : preview.status,
  ...
  const result = await query(
    `UPDATE xeivora_preview_versions
     SET
       ...
       deployed_at = COALESCE($9, deployed_at),
       updated_at = NOW()
     WHERE id = $1
     RETURNING *`,
    [
      ...
      payload.status !== undefined ? sanitizeText(payload.status) : null,
