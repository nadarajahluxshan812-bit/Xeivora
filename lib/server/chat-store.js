...
function loadStore() {
  ...
  try {
    ...
    return raw ? JSON.parse(raw) : deepClone(DEFAULT_STORE);
  } catch {
...
async function ensurePostgresSchema() {
  ...
  schemaReadyPromise ||= query(`
    ...
    -- Ownership added additively. Pre-existing sessions keep a NULL owner and
    -- stay shared/legacy (searchable for everyone); new sessions are stamped
    -- with their creator so search can scope results per viewer.
    ALTER TABLE xeivora_chat_sessions
...
async function createSession({ modelPreference = "orbit-auto", projectId = null, ownerId = null } = {}) {
  ...
  await query(
    `INSERT INTO xeivora_chat_sessions
      (id, title, preview, created_at, updated_at, model_preference, last_provider, route_label, pinned, archived, project_id, owner_id)
