...
function detectTools(prompt, files = []) {
  ...
  if (/\b(create|generate|make|design)\b.*\b(image|picture|photo|poster|illustration|logo|graphic|art|artwork)\b/.test(lower)) {
    ...
    if (tool === "image_generation") {
      ...
      executions.push({
        ...
        summary: result.connected
          ? `Generated ${result.images.length} image${result.images.length === 1 ? "" : "s"} with ${result.model || "OpenAI"}.`
