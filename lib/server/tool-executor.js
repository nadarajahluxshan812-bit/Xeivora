...
function looksLikeCodeAssistantPrompt(lower) {
  const codingKeywords =
    /\b(code|debug|bug|fix|refactor|implement|function|component|frontend|backend|api|database|schema|sql|next\.?js|react|typescript|javascript|python|tailwind|css|html|cli|terminal|script|readme|markdown|documentation|pdf|slides|ppt|image|video)\b/;
  const projectSurfaceKeywords =
    /\b(login|sign in|signup|sign up|auth|authentication|dashboard|sidebar|composer|message|conversation|page|screen|layout|header|footer|form|modal|preview|timeline|memory|project|calculator|landing page|readme)\b/;
...
// Strong software/coding signals only (deliberately excludes image/logo/icon/visual
// words so it never suppresses a genuine image request). Used as defense-in-depth so
// a clear coding task never gets routed to image generation.
function looksLikeSoftwareTask(lower) {
  return /\b(html|css|javascript|typescript|tsx|jsx|react|next\.?js|vue|svelte|angular|node\.?js|node|express|api|rest|graphql|endpoint|server|backend|frontend|database|sql|schema|migration|query|component|function|class|method|module|script|cli|terminal|bash|shell|python|java|rust|golang|php|ruby|webhook|crud)\b/.test(
    lower
  );
}

function inferCodePreviewMode(prompt = "") {
  ...
  if (/\b(pdf|reportlab|pdfkit|jspdf)\b/.test(lower)) {
    return "pdf";
  }

  if (/\b(slide|slides|ppt|pptx|powerpoint)\b/.test(lower)) {
    return "slides";
  }

  if (/\b(image|poster|illustration|logo|artwork)\b/.test(lower)) {
    return "image";
  }

  if (/\b(video|ffmpeg|moviepy|mp4)\b/.test(lower)) {
    return "video";
  }
...
function buildCodePreviewGuidance(prompt = "") {
  const lower = normalizePrompt(prompt).toLowerCase();
  if (!looksLikeCodeAssistantPrompt(lower)) {
    return null;
  }

  const mode = inferCodePreviewMode(prompt);

  if (mode === "markdown") {
    return [
      "Preview requirement:",
      "Return the documentation in a fenced ```markdown block so Xeivora can render the README preview inside chat.",
      "Keep the output as real markdown, not prose that describes markdown."
    ].join(" ");
  }
  ...
  if (mode === "database") {
    return [
      "Preview requirement:",
      "Return the database work in a fenced ```sql block so Xeivora can render a schema preview inside chat.",
      "Include CREATE TABLE statements and any useful sample query if relevant."
    ].join(" ");
  }
  ...
  if (mode === "api") {
    return [
      "Preview requirement:",
      "Return the backend/API code in fenced code blocks.",
      "Also include clearly inferable endpoints, plus a sample request body and sample JSON response when relevant, so Xeivora can render the API preview panel."
    ].join(" ");
  }
  ...
  if (mode === "terminal") {
    return [
      "Preview requirement:",
      "Return the script in a fenced code block for the requested language.",
      "Also include example console output or sample stdout when relevant so Xeivora can render a terminal preview instead of a blank panel."
    ].join(" ");
  }
  ...
  if (mode === "react") {
    return [
      "Preview requirement:",
      "Return the React or Next.js component in a fenced code block.",
      "Use a single primary component file when possible so Xeivora can attach it to the live preview shell."
    ].join(" ");
  }

  return [
    "Preview requirement:",
    "Return runnable UI code in fenced code blocks so Xeivora can render a live browser preview inside chat.",
    "Prefer a single ```html block for visual output.",
    "If you need styling or behavior, also include separate ```css and ```javascript blocks."
  ].join(" ");
}
...
function detectTools(prompt, files = []) {
  ...
  const imageRequested =
    imageIntent.isImageRequest ||
    /\b(create|generate|make|design)\b.*\b(image|picture|photo|poster|illustration|logo|graphic|art|artwork)\b/.test(lower);
  // Defense-in-depth: a clear software/coding task is never routed to image generation.
  if (imageRequested && !looksLikeSoftwareTask(lower)) {
  ...
  if (uniqueTools.includes("code_assistant")) {
    return uniqueTools.filter((tool) => tool !== "document_writer");
  }
...
function inferPreviewTitle(prompt = "", routePath = "/") {
  ...
  if (routePath === "/memory") {
    return "Expanded Project Memory";
  }
...
async function executeTools({ prompt, sessionId = null, projectId = null, files = [], viewerId = null, viewerPlan = "Starter" }) {
  ...
  const imageIntent = parseImageIntent(prompt);
  const promptLooksLikeSoftware = looksLikeSoftwareTask(normalizePrompt(prompt).toLowerCase());
  ...
  for (const tool of tools) {
    ...
    if (tool === "image_generation") {
      ...
      if (!result.connected && !promptLooksLikeSoftware) {
        // Only dead-end on the image error for genuine image prompts. If the prompt
        // also looks like software, let the coding model respond and produce code.
    ...
    if (tool === "code_assistant") {
      ...
      const previewGuidance = buildCodePreviewGuidance(prompt);
      ...
      promptAugmentation = [promptAugmentation, previewGuidance].filter(Boolean).join("\n\n") || promptAugmentation;

      executions.push({
        tool,
        connected: true,
        summary: `Live Preview opened. Version ${previewVersion.versionNumber} tracking this checkpoint.`,
        payload: {
          previewVersion,
          previewHref: previewParams.toString() ? `/chat?${previewParams.toString()}&preview=1` : "/chat?preview=1",
          routePath
        },
        logPayload: {
