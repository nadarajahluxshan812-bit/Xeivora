...
function looksLikeVisualPreviewPrompt(lower) {
  return /\b(ui|ux|page|screen|layout|component|frontend|html|css|tailwind|react|next\.?js|app|website|landing|dashboard|sidebar|modal|form|button|card|calculator|preview)\b/.test(
    lower
  );
}
...
function buildCodePreviewGuidance(prompt = "") {
  ...
  if (!looksLikeVisualPreviewPrompt(lower)) {
    return null;
  }

  return [
    "Preview requirement:",
    "Return runnable UI code in fenced code blocks so Xeivora can render a live preview inside chat.",
    ...
    "Avoid returning only React or Next.js component code unless the user explicitly asks for framework-specific output."
  ].join(" ");
}
  ...
  for (const tool of tools) {
    ...
    if (tool === "code_assistant") {
      ...
      const previewVersion = await createPreviewVersion({
        ...
        sessionId,
        title,
        summary: "Live Preview opened. Waiting for renderable output.",
        routePath,
