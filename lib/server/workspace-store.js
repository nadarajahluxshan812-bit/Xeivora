...
async function searchWorkspace(rawQuery) {
  ...
  const mvpStore = require("./mvp-store");
  const [sessions, projects, files, memories, previews] = await Promise.all([
  ...
  memories.forEach((memory) => {
    ...
    if (haystack.includes(queryText)) {
      ...
      results.push({
        ...
        href: memory.projectId ? `/memory?project=${memory.projectId}` : "/memory",
  ...
  previews.forEach((preview) => {
    ...
    if (haystack.includes(queryText)) {
      results.push({
        ...
        href: preview.projectId ? `/timeline?project=${preview.projectId}` : "/timeline",
        projectId: preview.projectId || null,
        updatedAt: preview.updatedAt || preview.createdAt || now()
      });
    }
  });
