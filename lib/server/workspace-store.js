...
// Ownership-aware global search. Results are scoped to the projects the viewer
// may see (owner-less legacy projects + projects they own); data tied to other
// users' projects is never returned. Callers must pass the resolved viewerId
// (the /api/search route enforces authentication before calling).
async function searchWorkspace(rawQuery, { viewerId = null } = {}) {
  ...
  const { listSessions } = require("./chat-store");
  const { listPreviewVersions } = require("./preview-store");
  const mvpStore = require("./mvp-store");
  const { buildScopedResults } = require("../search-scope");

  const [sessions, visibleProjects, files, memories, previews] = await Promise.all([
    listSessions({ includeArchived: true }),
    listVisibleProjects(viewerId),
    listFiles({ limit: 80 }),
    mvpStore.list("memory"),
    listPreviewVersions({ limit: 80 })
  ]);

  return buildScopedResults({
    query: queryText,
    visibleProjects,
    sessions,
    files,
