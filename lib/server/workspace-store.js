...
async function createUploadedFile(payload = {}) {
  ...
  await query(
    `INSERT INTO xeivora_uploaded_files
      (id, session_id, project_id, name, mime_type, kind, size_bytes, storage_path, preview_text, summary, extracted_text, analysis_status, metadata, created_at, updated_at)
...
async function searchWorkspace(rawQuery, { viewerId = null } = {}) {
  ...
  // Everything is scoped to the projects the viewer may see. `projects` is
  // already owner-filtered; session/file/memory records are kept only when
  // they belong to one of those projects. Records with no project (or a
  // project owned by someone else) are excluded, so search never leaks another
  // user's chats, files, or memories.
  const visibleProjectIds = new Set(projects.map((project) => project.id));
  const belongsToVisibleProject = (projectId) =>
    Boolean(projectId) && visibleProjectIds.has(projectId);

  const results = [];

  sessions.forEach((session) => {
    if (!belongsToVisibleProject(session.projectId)) {
      return;
    }
  ...
  files.forEach((file) => {
    if (!belongsToVisibleProject(file.projectId)) {
      return;
    }
  ...
  memories.forEach((memory) => {
    if (!belongsToVisibleProject(memory.projectId)) {
      return;
    }
