 ...
 * chat/file/memory to each, then asserts that searchWorkspace() only returns
 * records belonging to projects the viewer may see.
 *
...
async function seedProject(ownerId, label) {
  ...
  const session = await chat.createSession({ projectId: project.id });
  await chat.updateSessionMetadata(session.id, { title: `Chat ${label} ${TOKEN}` });
  await ws.createUploadedFile({ name: `file-${label}-${TOKEN}.txt`, projectId: project.id, storagePath: "x" });
  await mvp.create("memory", { title: `Memory ${label} ${TOKEN}`, content: "note", projectId: project.id });
  return project;
}
...
async function main() {
  ...
  // Viewer A searches: should see only A's records + the legacy project's.
  ...
  // No result of any category may reference B's project.
  const allHrefsA = JSON.stringify(resultsA);
  check("A's results contain no chat from B", idsByCategory(resultsA, "chat").length >= 2);
  ...
  // Every returned chat/file/memory must belong to a project A returned.
  const visibleProjectIdsA = new Set(projectIdsA);
  const sessionsA = await chat.listSessions({ includeArchived: true });
  const sessionProject = new Map(sessionsA.map((s) => [s.id, s.projectId]));
  const strayChat = idsByCategory(resultsA, "chat").find(
    (id) => !visibleProjectIdsA.has(sessionProject.get(id))
  );
  check("A's chat results all belong to visible projects", !strayChat);
